...
ref_dec = np.array(rng.uniform(-90, 90) * np.pi / 180)
ref_ra = np.array(rng.uniform(0, 360) * np.pi / 180)
ref_phi = ref_ra
...
for i in range(1, steps + 1):
    ...
    dec_rotation_matrix = np.linalg.multi_dot(
        [
...
for i in range(10):
    ra_sample = np.array(rng.uniform(0, 2 * np.pi))
    dec_sample = np.array(rng.uniform(-np.pi / 2, np.pi / 2))
